  ...
  <div class="slider-wrapper theme-default">
    <div id="slider" class="nivoSlider">
        ...
        while ( $query->have_posts() ) : $query->the_post();
          $i++;
          $foundations_slideno[] = $i;
          $foundations_slidetitle[] = get_the_title();
		  $foundations_slidedesc[] = get_the_excerpt();
          $foundations_slidelink[] = esc_url(get_permalink());
          ?>
        ...
        <?php
        $k = 0;
        foreach( $foundations_slideno as $foundations_sln ){ ?>
    <div id="slidecaption<?php echo esc_attr( $foundations_sln ); ?>" class="nivo-html-caption">
      <div class="top-bar">
        <h2><a href="<?php echo esc_url($foundations_slidelink[$k] ); ?>"><?php echo esc_html($foundations_slidetitle[$k] ); ?></a></h2>
        <p><?php echo esc_html($foundations_slidedesc[$k] ); ?></p>
        <div class="clear"></div>
        <a class="slide-button" href="<?php echo esc_url($foundations_slidelink[$k] ); ?>">
          <?php echo esc_html(get_theme_mod('slide_text',__('Get in Touch','foundations')));?>
          </a>
      </div>
    </div>
 	<?php $k++;
