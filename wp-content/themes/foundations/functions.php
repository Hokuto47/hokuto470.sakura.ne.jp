...
function foundations_setup() {
	...
	add_image_size('foundations-homepage-thumb',true);
	...
	add_editor_style( array( 'editor-style.css', foundations_font_url() ) );
}
...
function foundations_widgets_init() {
	register_sidebar( array(
		'name'          => __( 'Blog Sidebar', 'foundations' ),
		'description'   => __( 'Appears on blog page sidebar', 'foundations' ),
		'id'            => 'sidebar-1',
...
function foundations_font_url(){
		...
		*/
		$karla = _x('on', 'Karla font:on or off','foundations');
		
		/* Translators: If there are any character that are
		* not supported by Montserrat, translate this to off, do not
		* translate into your own language.
		*/
		$mont = _x('on', 'Montserrat font:on or off','foundations');
...
function foundations_scripts() {
	...
	wp_enqueue_style( 'foundations-basic-style', get_stylesheet_uri() );
	wp_enqueue_style( 'foundations-responsive-style', get_template_directory_uri().'/css/theme-responsive.css' );
	...
	wp_enqueue_script( 'foundations-customscripts', get_template_directory_uri() . '/js/custom.js', array('jquery') );
